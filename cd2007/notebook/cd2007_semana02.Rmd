...
Check on the two projects described in the file `cd2007_semana02_cases.pdf` as a group activity:

* One case is **procedural**, to get familiar with `R` and `lpSolve`

* Another is **conceptual**, to get familiar with the **model specification of a problem**.

# Next week 

A very **important question emerges** now:

> What if I/we cannot achieve the `coordinates that maximize` the `objective function`?

> How can I maesure the impact induced by those `deviations`?

The answer to those questions will be addressed though the so called **sensitivity analysis**.
